...
type TagsSelectProps = SelectHTMLAttributes<HTMLSelectElement> & {
  selectedTags: string[];
  ...
  return (
    <>
      <select
        ...
        title="Tags"
        name="tags"
        className="px-4 py-2 border rounded-md capitalize w-full dark:bg-black dark:border-gray-800"
        {...props}
      >
        <option className="capitalize" value="">
          {tagsStatus === "loading" ? "Loading..." : "Select Tag"}
        </option>
        {availableTags.map((tag) => (
          <option
            ...
            className={`capitalize p-4 ${
              isTagSelected(tag.id) && "bg-blue-400 text-white"
            }`}
          >
            {tag.tag}
          </option>
